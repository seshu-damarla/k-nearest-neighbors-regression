...
De = zeros(M,M); % Initializes the matrix of e-distances.
for i = 1:M-1
xa = X(i,:);
ya = y(i,:);
ea = e(i,:);
...
Dymax = max(max(Dy));
Demax = max(max(De));
D= Dx/Dxmax + Dy/Dymax+ De/Demax; % Combines the X, y and e distances.
% D is an upper triangular matrix.
% D(i,j) is the distance between samples i and j (j > i).
